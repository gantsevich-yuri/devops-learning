...
**Горизонтальный шардинг** - деление таблицы по строкам.
База данных состоит из трёх таблиц:

пользователи:
users(id, name, family, city)
...
**Вертикальный шардинг** - деление таблицы по столбцам.

Таблица пользователи:
Первый шард: id, name, family
Второй шард: id, city
...
Можно использовать один  master-сервер (для чтения и записи) и один slave-сервер(для чтения) на каждый шард.
Итого 4 серверов в минимальной отказоустойчивой конфигурации.
